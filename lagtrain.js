...
let lf = 3777; // last frame (could use fs to auto detect this, but i.m too lazy)
let scale_const = 2; // How many times will the original scale be divided
let threshold = 150; // above what rgb value (0-255) to consider a 'pixel' white

async function magicFunction (imgId) {
    await jimp.read('./frames_l/' + imgId + '.png', async (err, img) => {
        if (err) throw err;
